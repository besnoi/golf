--[[
	AutoCursor Library (edited for this particular game)
	By Neer
]]
...
function fcursor.update(dt)
	if not fcursor.destinationReached() then
		...
		if math.floor(math.abs(fcursor.destx-fcursor.x))<fcursor.dx then fcursor.x=fcursor.destx end
		...
		if math.floor(math.abs(fcursor.desty-fcursor.y))<fcursor.dy then fcursor.y=fcursor.desty end
	end
